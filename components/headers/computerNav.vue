...
<div id="header">
  <header class="tony-header-fixed" id="header-div">
    ...
    <div class="header-div2">
      ...
      <!-- Contact Dropdown -->
      <b-dropdown variant="primary" split :split-href="lang === 'zh-CN' ? '/post/126' : '/bio'" :html="$t('lang.header.aboutme')">
        <b-dropdown-item href="https://github.com/HelipengTony">Github</b-dropdown-item>
        <b-dropdown-item href="http://wpa.qq.com/msgrd?v=3&amp;uin=36624065&amp;site=qq&amp;menu=yes">QQ</b-dropdown-item>
      ...
      <div class="search-bg">
        <div class="search-div1">
          <h3>
            ...
            <button type="button" class="btn btn-primary" style="font-weight: 600;padding: 4px 14px;font-size: .9rem;margin-top: 6px;margin-left: 10px;float: right;" @click="closeSearch">{{ $t('lang.header.searchDiv.close') }}</button>
          </h3>
          <p>{{ $t('lang.header.searchDiv.des') }}</p>
          <input class="uk-input" type="text" :placeholder="$t('lang.header.searchDiv.placeholder')" v-on:keyup.enter="searchQuery" v-model.trim="search_key" />
        </div>
  ...
  <div id="view-div" class="center-info" style="display:none" v-if="!($route.path).match(listTextDetect)">
    <p style="font-weight: 600;font-size: 1.2rem;color: #555;" id="view-text">-&nbsp;{{ $t('lang.header.postlist') }}&nbsp;-</p>
  </div>
</div>
</template>
