<template>
  <div id="header_info" class="index-top">
    <nav class="header-nav reveal">
      <img
        class="header-avatar-top"
        src="https://static.ouorz.com/t.jpg"
        loading="lazy"
        alt="TonyHe's Avatar"
      />
      <a
    ...
    <!-- 英文版长内容展示不同样式 -->
    <div
      :class="'index-cates ' + ($i18n.locale == 'en-US' ? 'index-long' : '')"
    >
      <template v-if="loading_cates">
        <ContentLoader height="35" width="660" speed="1">
          <rect x="10" y="0" rx="3" ry="3" width="660" height="35" />
        </ContentLoader>
      </template>
      <template v-else>
    ...
    <div class="tags-div">
      ...
      <ul :class="'post_tags ' + (cookie ? 'post_tags_noscroll' : '')">
        ...
        <template v-else>
          <ContentLoader height="25" width="660" speed="1">
            <rect x="0" y="0" rx="3" ry="3" width="660" height="25" />
          </ContentLoader>
        </template>
      </ul>
